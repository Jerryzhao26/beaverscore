...
export const SyncNotificationModal: React.FC<SyncNotificationModalProps> = ({
  ...
  const hasScoreChanges = (notification.incomingScoresCount && notification.incomingScoresCount > 0) ||
    ...
    (notification.incomingScoresUpdated && notification.incomingScoresUpdated > 0) ||
    (notification.outgoingScoresUpdated && notification.outgoingScoresUpdated > 0);

  const hasStudentLevelOrProfileUpdates = (notification.outgoingStudentUpdates && notification.outgoingStudentUpdates.length > 0) ||
    (notification.incomingStudentUpdates && notification.incomingStudentUpdates.length > 0) ||
    (notification.outgoingStudentsUpdated && notification.outgoingStudentsUpdated > 0) ||
    (notification.incomingStudentsUpdated && notification.incomingStudentsUpdated > 0);

  const hasStudentChanges = (notification.incomingStudentsCount && notification.incomingStudentsCount > 0) ||
    (notification.outgoingStudentsCount && notification.outgoingStudentsCount > 0) ||
    hasStudentLevelOrProfileUpdates;

  const hasClassChanges = (notification.incomingClassesCount && notification.incomingClassesCount > 0) ||
    (notification.outgoingClassesCount && notification.outgoingClassesCount > 0) ||
    (notification.outgoingClassUpdates && notification.outgoingClassUpdates.length > 0) ||
    (notification.incomingClassUpdates && notification.incomingClassUpdates.length > 0);
  ...
  const getHeaderBadge = () => {
    ...
    if (notification.action === 'push') {
      return <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 border border-blue-200">云端推送 · 智能合并</span>;
    }
    if (notification.action === 'pull') {
      return <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 border border-emerald-200">云端拉取 · 刷新数据</span>;
    }
  ...
  // Combine student updates
  const allStudentUpdates = [
    ...(notification.outgoingStudentUpdates || []).map(u => ({ ...u, direction: 'outgoing' as const })),
    ...(notification.incomingStudentUpdates || []).map(u => ({ ...u, direction: 'incoming' as const }))
  ];

  // Combine class updates
  const allClassUpdates = [
    ...(notification.outgoingClassUpdates || []).map(u => ({ ...u, direction: 'outgoing' as const })),
    ...(notification.incomingClassUpdates || []).map(u => ({ ...u, direction: 'incoming' as const }))
  ];
  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden transform transition-all duration-200 scale-100"
      ...
      >
        ...
        <div className="px-6 py-5 overflow-y-auto space-y-4 flex-1">
          {/* Main Description */}
          <div className="text-sm text-slate-700 leading-relaxed bg-slate-50 p-3.5 rounded-xl border border-slate-100 flex items-start gap-2">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              {/* Student Updates Pill */}
              {(notification.outgoingStudentsUpdated && notification.outgoingStudentsUpdated > 0) ? (
                <div className="bg-indigo-50/80 border border-indigo-200 rounded-xl p-3 text-center">
                  <div className="text-xs text-indigo-800 font-bold flex items-center justify-center gap-1">
                    <Award className="w-3.5 h-3.5 text-indigo-600" />
                    已同步学员级别
                  </div>
                  <div className="text-xl font-extrabold text-indigo-700 mt-0.5">
                    {notification.outgoingStudentsUpdated} <span className="text-xs font-normal">人</span>
                  </div>
                </div>
              ) : (notification.incomingStudentsUpdated && notification.incomingStudentsUpdated > 0) ? (
                <div className="bg-purple-50/80 border border-purple-200 rounded-xl p-3 text-center">
                  <div className="text-xs text-purple-800 font-bold flex items-center justify-center gap-1">
                    <Award className="w-3.5 h-3.5 text-purple-600" />
                    云端更新级别
                  </div>
                  <div className="text-xl font-extrabold text-purple-700 mt-0.5">
                    {notification.incomingStudentsUpdated} <span className="text-xs font-normal">人</span>
                  </div>
                </div>
              ) : null}

              {notification.outgoingScoresCount !== undefined && notification.outgoingScoresCount > 0 && (
                <div className="bg-blue-50/80 border border-blue-100 rounded-xl p-3 text-center">
                  ...
                  <div className="text-xl font-extrabold text-blue-700 mt-0.5">
                    +{notification.outgoingScoresCount} <span className="text-xs font-normal">条</span>
                  </div>
                </div>
              )}

              {notification.incomingScoresCount !== undefined && notification.incomingScoresCount > 0 && (
                <div className="bg-emerald-50/80 border border-emerald-100 rounded-xl p-3 text-center">
                  ...
                  <div className="text-xl font-extrabold text-emerald-700 mt-0.5">
                    +{notification.incomingScoresCount} <span className="text-xs font-normal">条</span>
                  </div>
                </div>
              )}

              {notification.incomingStudentsCount !== undefined && notification.incomingStudentsCount > 0 && (
                <div className="bg-amber-50/80 border border-amber-200 rounded-xl p-3 text-center">
                  <div className="text-xs text-amber-800 font-medium">新加入学员</div>
                  <div className="text-xl font-extrabold text-amber-700 mt-0.5">
                    +{notification.incomingStudentsCount} <span className="text-xs font-normal">人</span>
                  </div>
                </div>
              )}

              {notification.totalStudentsCount !== undefined && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-center">
                  <div className="text-xs text-slate-500 font-medium">全校在册学员</div>
                  <div className="text-xl font-extrabold text-slate-800 mt-0.5">
                    {notification.totalStudentsCount} <span className="text-xs font-normal">人</span>
                  </div>
                </div>
              )}
              ...
              {notification.totalScoresCount !== undefined && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-center">
                  <div className="text-xs text-slate-500 font-medium">全校总考评记录</div>
                  <div className="text-xl font-extrabold text-slate-800 mt-0.5">
                    {notification.totalScoresCount} <span className="text-xs font-normal">条</span>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* 🏷️ Detailed Student Level & Profile Changes (CRITICAL HIGHLIGHT) */}
          {allStudentUpdates.length > 0 && (
            <div className="bg-gradient-to-br from-indigo-50/80 via-white to-purple-50/50 border border-indigo-200 rounded-xl p-4 space-y-2.5 shadow-xs">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-xs font-bold text-indigo-950">
                  <Award className="w-4 h-4 text-indigo-600" />
                  学员级别与档案变更明细 ({allStudentUpdates.length} 条)
                </div>
                {onNavigateToStudents && (
                  <button
                    onClick={() => {
                  ...
                  >
                    前往档案花名册 <ArrowRight className="w-3 h-3" />
                  </button>
                )}
              </div>

              <div className="divide-y divide-indigo-100/80 border border-indigo-100 rounded-lg overflow-hidden bg-white">
                {allStudentUpdates.map((update, idx) => (
                  <div key={idx} className="p-3 text-xs flex items-center justify-between gap-3 hover:bg-indigo-50/40 transition">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 flex items-center gap-1.5">
                          ...
                          <span className={`text-[10px] px-1.5 py-0.2 rounded font-normal ${
                            ...
                            {update.direction === 'outgoing' ? '本地修改' : '云端同步'}
                          </span>
                        </div>
                    ...
                    {update.from && update.to && update.field === 'currentLevel' && (
          ...
          {allClassUpdates.length > 0 && (
            <div className="bg-blue-50/50 border border-blue-200 rounded-xl p-3.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs font-bold text-blue-950">
                <School className="w-4 h-4 text-blue-600" />
                班级级别与设置调整 ({allClassUpdates.length} 个)
              </div>
              <div className="divide-y divide-blue-100 border border-blue-100 rounded-lg overflow-hidden bg-white">
                {allClassUpdates.map((cUpdate, idx) => (
        ...
        <div className="px-6 py-3.5 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={onClose}
              className="px-5 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 rounded-xl transition-all shadow-xs cursor-pointer"
            >
              确认并关闭
            </button>
          </div>
